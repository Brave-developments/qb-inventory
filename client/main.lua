    ...
    if not isCrafting and not inInventory then
        if not PlayerData.metadata["isdead"] and not PlayerData.metadata["inlaststand"] and not PlayerData.metadata["ishandcuffed"] and not IsPauseMenuActive() then
            ...
            if CurrentVehicle then -- Trunk
                ...
                if not trunkConfig then return print('Araç bagajı yapılandırması alınamıyor') end
                local slots = trunkConfig.slots
                local maxweight = trunkConfig.maxWeight
                if not slots or not maxweight then return print('Araç yuvaları ve maksimum ağırlık alınamıyor') end
...
RegisterNUICallback("GiveItem", function(data, cb)
    ...
    if player ~= -1 and distance < 3 then
        if data.inventory == 'player' then
            ...
            QBCore.Functions.Notify("You do not have this item!", "error")
        end
    else
...
RegisterNetEvent("qb-burgershot:GiftBox", function()
    ...
    QBCore.Functions.Progressbar("pickup", "The Gift Box is opening..", 4000, false, true, {
        disableMovement = true,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = false,
    ...
    --remove box
    Citizen.Wait(1000)
    TriggerServerEvent('burgershot:server:RemoveItem', "gift_box",1)
    TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items["gift_box"], "remove")
    --add items from box
    Citizen.Wait(3000)
    TriggerServerEvent('burgershot:server:AddItem', "phone",1)
    TriggerServerEvent('burgershot:server:AddItem', "sandwich",5)
    TriggerServerEvent('burgershot:server:AddItem', "water_bottle",5)
    ...
    if randomToy < 4 then
        QBCore.Functions.Notify("You didn't receive an extra gift.", "error")
    elseif randomToy == 4 then
        TriggerServerEvent('burgershot:server:AddItem', "lockpick")
    ...
    elseif randomToy == 10 then
        TriggerServerEvent('burgershot:server:AddItem', "screwdriverset")
