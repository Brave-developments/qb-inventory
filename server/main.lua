...
local function AddItem(source, item, amount, slot, info, created)
    ...
    -- Debugging: Log the item name if it is missing
    if not itemInfo then
        print("Error: Item does not exist in QBCore.Shared.Items. Item Name:", item)
        QBCore.Functions.Notify(source, "Item does not exist: " .. tostring(item), 'error')
        return false
    end
...
QBCore.Commands.Add("resetinv", "Reset Inventory (Admin Only)", {{name="type", help="stash/trunk/glovebox"},{name="id/plate", help="ID of stash or license plate"}}, true, function(source, args)
	...
	if invType and invId then
		...
		elseif invType == "stash" then
			...
			TriggerClientEvent('QBCore:Notify', source,  "Geçerli değil..", "error")
		end
	else
		TriggerClientEvent('QBCore:Notify', source,  "Lütfen doğru şekilde yazın ", "error")
	end
end, "admin")
...
QBCore.Commands.Add("giveitem", "İtem ver (Admin Only)", {{name="id", help="Oyuncu ID"},{name="item", help="item kodu (ismi değil)"}, {name="amount", help="İtem miktarı"}}, false, function(source, args)
	...
	if Player then
			if itemData then
				...
				if AddItem(id, itemData["name"], amount, false, info) then
					...
					QBCore.Functions.Notify(source, "Unable to give the item!", "error")
				end
				else
					QBCore.Functions.Notify(source, "Item Not Available", "error")
				end
				else
...
QBCore.Commands.Add('clearinv', 'Oyuncunun envanterini temizle (Admin Only)', { { name = 'id', help = 'Oyuncu ID' } }, false, function(source, args)
    local playerId = args[1] ~= '' and tonumber(args[1]) or source
    local Player = QBCore.Functions.GetPlayer(playerId)
    if Player then
        ClearInventory(playerId)
    else
        QBCore.Functions.Notify(source, "Player is not active", 'error')
    end
end, 'admin')
...
RegisterServerEvent('burgershot:server:RemoveItem', function(item,amount)
    local src = source
    local ply = QBCore.Functions.GetPlayer(src)
    ply.Functions.RemoveItem(item,amount)
end)

RegisterServerEvent('burgershot:server:AddItem', function(item,amount)
    local src = source
    local ply = QBCore.Functions.GetPlayer(src)
